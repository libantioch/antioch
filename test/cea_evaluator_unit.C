...
Scalar h( Scalar T, Scalar a0, Scalar a1, Scalar a2, 
          ...
          Scalar a8 )
{
  return -a0/(T*T) + a1*std::log(T)/T + a2 + a3*T/2.0 + a4*(T*T)/3.0 + a5*(T*T*T)/4.0 + a6*(T*T*T*T)/5.0 + a8/T;
}
...
int tester()
{
  ...
  // Test N2 cp
  {
    ...
    return_flag_temp = test_cp( species_name, index, cp_N2_2, T2, thermo );
    if( return_flag_temp != 0 ) return_flag = 1;

    return_flag_temp = test_cp( species_name, index, cp_N2_3, T3, thermo );
    if( return_flag_temp != 0 ) return_flag = 1;
  }
  ...
  {
    unsigned int index = 1;
    const Scalar h_1 = R_O2*T1*h( T1, Scalar(-3.42556269e+04), Scalar(4.84699986e+02), Scalar(1.11901159e+00), 
                                  Scalar(4.29388743e-03), Scalar(-6.83627313e-07), Scalar(-2.02337478e-09),
                                  Scalar(1.03904064e-12), Scalar(-3.39145434e+03) );

    const Scalar h_2 = R_O2*T2*h( T2, Scalar(-1.03793994e+06), Scalar(2.34483275e+03), Scalar(1.81972949e+00),
                                  Scalar(1.26784887e-03), Scalar(-2.18807142e-07), Scalar(2.05372411e-11),
                                  Scalar(-8.19349062e-16), Scalar(-1.68901253e+04) );

    const Scalar h_3 = R_O2*T3*h( T3, Scalar(4.97515261e+08), Scalar(-2.86602339e+05), Scalar(6.69015464e+01),
                                  Scalar(-6.16971869e-03), Scalar(3.01623757e-07), Scalar(-7.42087888e-12),
                                  Scalar(7.27744063e-17), Scalar(2.29348755e+06) );
  ...
  {
    unsigned int index = 2;
    const Scalar h_1 = R_N*T1*h( T1, Scalar(0.00000000e+00), Scalar(0.00000000e+00), Scalar(2.50000000e+00),
                                 Scalar(0.00000000e+00), Scalar(0.00000000e+00), Scalar(0.00000000e+00),
                                 Scalar(0.00000000e+00), Scalar(5.61046378e+04) );

    const Scalar h_2 = R_N*T2*h( T2, Scalar(8.87650138e+04), Scalar(-1.07123150e+02), Scalar(2.36218829e+00),
                                 Scalar(2.91672008e-04), Scalar(-1.72951510e-07), Scalar(4.01265788e-11),
                                 Scalar(-2.67722757e-15), Scalar(5.69735133e+04) );

    const Scalar h_3 = R_N*T3*h( T3, Scalar(5.47518105e+08), Scalar(-3.10757498e+05), Scalar(6.91678274e+01),
                                 Scalar(-6.84798813e-03), Scalar(3.82757240e-07), Scalar(-1.09836771e-11),
                                 Scalar(1.27798602e-16), Scalar(2.55058562e+06) );
  ...
  {
    unsigned int index = 3;
    const Scalar h_1 = R_O*T1*h( T1, Scalar(-7.95361130e+03) , Scalar(1.60717779e+02), Scalar(1.96622644e+00),
                                 Scalar(1.01367031e-03), Scalar(-1.11041542e-06), Scalar(6.51750750e-10),
                                 Scalar(-1.58477925e-13), Scalar(2.84036244e+04) );

    const Scalar h_2 = R_O*T2*h( T2, Scalar(2.61902026e+05), Scalar(-7.29872203e+02), Scalar(3.31717727e+00),
                                 Scalar(-4.28133436e-04), Scalar(1.03610459e-07), Scalar(-9.43830433e-12),
                                 Scalar(2.72503830e-16), Scalar(3.39242806e+04) );

    const Scalar h_3 = R_O*T3*h( T3, Scalar(1.77900426e+08), Scalar(-1.08232826e+05), Scalar(2.81077837e+01),
                                 Scalar(-2.97523226e-03), Scalar(1.85499753e-07), Scalar(-5.79623154e-12),
                                 Scalar(7.19172016e-17), Scalar(8.89094263e+05) );
  ...
  {
    unsigned int index = 4;
    const Scalar h_1 = R_NO*T1*h( T1, Scalar(-1.14391658e+04), Scalar(1.53646774e+02), Scalar(3.43146865e+00),
                                  Scalar(-2.66859213e-03), Scalar(8.48139877e-06), Scalar(-7.68511079e-09),
                                  Scalar(2.38679758e-12), Scalar(9.09794974e+03) );

    const Scalar h_2 = R_NO*T2*h( T2, Scalar(2.23903708e+05), Scalar(-1.28965624e+03), Scalar(5.43394039e+00),
                                  Scalar(-3.65605546e-04), Scalar(9.88101763e-08), Scalar(-1.41608327e-11),
                                  Scalar(9.38021642e-16), Scalar(1.75029422e+04) );

    const Scalar h_3 = R_NO*T3*h( T3, Scalar(-9.57530764e+08), Scalar(5.91243671e+05), Scalar(-1.38456733e+02),
                                  Scalar(1.69433998e-02), Scalar(-1.00735146e-06), Scalar(2.91258526e-11),
                                  Scalar(-3.29511091e-16), Scalar(-4.67750329e+06) );
