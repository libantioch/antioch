...
int tester(const std::string& input_name)
{
  ...
  eq_solver_TP.first_guess_mass_fraction(first);
  eq_solver_TP.equilibrium();

  // solution testing
  std::vector<Scalar> Y_eq = eq_solver_TP.mass_fraction_equilibrium();
  ...
  chem_mixture.molar_densities(rho,Y_eq,molar_densities);
  ...
  for( unsigned int s = 0; s < n_species; s++)
   ...
   {
     return_flag = 1;
     std::cout << "equilibrium failed (" << eq_solver_TP.max_loop_tol() << "):\n\tsum " 
               << eq_solver_TP.residual() << ", threshold: " << eq_solver_TP.conv_threshold() << std::endl;
   }
