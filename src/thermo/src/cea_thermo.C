...
namespace Antioch
{
  ...
  template<class NumericType>
  void CEAThermodynamics<NumericType>::read_thermodynamic_table( std::istream& in )
  {
	...
	if( it != _chem_mixture.species_list().end() )
	  {
	    antioch_assert_equal_to( _chem_mixture.species_inverse_name_map().find(species)->second, name );
	    unsigned int index = static_cast<unsigned int>(it - _chem_mixture.species_list().begin());
		
	    _species_curve_fits[index] = new CEACurveFit<NumericType>(coeffs);
	  }
  ...
  template<class NumericType>
  void CEAThermodynamics<NumericType>::read_thermodynamic_table()
  {
