...
namespace Antioch
{
  ...
  /*
     !\class PhysicalSet
      ...
      Fork between diffusion, viscosity and thermal conduction. There are
      several things to define:
        - the set of the quantity to be computed, a value, a vector of values, a matrix of values
        - the computation method:
                - one value: viscosity, diffusivity or thermal_conductivity
                - a array of value: viscosities, diffusivities or thermal_conductivities
                - in case of kinetics theory diffusion, either a matrix of binary
                  value, binary_diffusion_matrix, or the self-diffusion, self_diffusion.
 
     The common part, initialization, adding a model and desctruction, are managed
     by generic programming using traits (tags).
   */
...
// always usable wherever
        ...
        template<typename StateType>
        void compute_diffusivity(const StateType & rho, const StateType & cp, const StateType & k, StateType & ds) const;
  ...
  template<typename StateType>
  inline
  void PhysicalSet<Physics,Mixture>::compute_diffusivity(const StateType & rho, const StateType & cp, const StateType & k, StateType & ds) const
  {
