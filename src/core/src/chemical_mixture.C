...
namespace Antioch
{
  ...
  template<class NumericType>
  void ChemicalMixture<NumericType>::read_species_data( std::istream& in )
  {
	...
	if (in.good())
	  {
		...
		_chemical_species[index] = new ChemicalSpecies<NumericType>(name, mol_wght, h_form, n_tr_dofs, charge);
	      }
  ...
  template<class NumericType>
  void ChemicalMixture<NumericType>::read_species_data()
  {
    /*!
